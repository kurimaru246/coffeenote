...
if(!empty($_POST)){
  ...
  if(empty($dbFormData)){ //DB情報が空だった場合（新規登録の場合）
    //未入力チェック
    ...
    if($dbFormData['name'] !== $name){ //DB情報と商品名の入力情報が違かった場合
      //未入力チェック
      validRequired($name,'name');
      //最大文字数チェック
      validMaxLen($name,'name');
    }if($dbFormData['category_id'] !== $category){ //DB情報とカテゴリの入力情報が違かった場合
      //セレクトボックスチェック
      validSelect($category,'category_id');
    }
    if($dbFormData['comment'] !== $comment){ //DB情報と詳細の入力情報が違かった場合
      //最大文字数チェック
      validMaxLen($comment,'comment', 500);
    }
    if($dbFormData['price'] !== $price){ //DB情報と金額の入力情報が違かった場合,前回まではキャストしていたが、ゆるい判定でもいい
      //未入力チェック
      validRequired($price,'price');
      //半角数字チェック
      validNumber($price,'price');
...
<body class="registProduct page-2colum page-logined">
...
?>
    ...
    <section id="main">
      <div class="form-container">
        <form action="" method="post" class="form-product" enctype="multipart/form-data">
          <div class="area-msg">
            <?php
          ...
          <label class="<?php if(!empty($err_msg['category_id'])) echo 'err'; ?>">
            ...
            <select class="category" name="category_id" id="">
          ...
          <div class="btn-container">
            <input type="submit" class="btn" value="<?php echo (!$edit_flg) ? '出品する' : '更新する'; ?>">
          </div>
        </form>
