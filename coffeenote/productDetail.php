...
if(!empty($_POST['submit'])){
  ...
  try{
    ...
    if($stmt){
      ...
      debug('連絡掲示板へ遷移します。');
      //連絡掲示板も誰と誰がやり取りするのかで出す連絡掲示板ID(m_id)が違うので、それを指定してあげる
...
<!--         iタグでフォントオーサムのアイコンを追加-->
<!--         isLike関数でuser_idとproductのidをDBに渡して、レコードがあるかどうかを確認-->
<!--         trueかfalseで返事が返ってくるので、trueだったらactiveにする-->
          <i class="fas fa-heart icn-like js-click-like <?php if(isLike($_SESSION['user_id'], $viewData['id'])){ echo 'active'; } ?>" aria-hidden="true" data-productid="<?php echo sanitize($viewData['id']); ?>" ></i>
        </div>
...
<!--           トップページに戻る際にURLを指定してあげる。こうすると、商品一覧で2ページ目以降の商品から商品詳細へ飛んで、そこからトップページに戻ってももとのページに戻るようになる-->
<!--           function.phpにてreturnで返しているだけなので必ずechoをつけて出力できるようにしておく-->
            <a href="index.php<?php echo appendGetParam(array('p_id')); ?>">&lt; 商品一覧へ戻る</a>
          </div>
          <form action="" method="post"><!--formタグを追加し、ボタンをinputに変更してstyle追加-->
          <div class="item item-right">
            <input type="submit" value="BUY" name="submit" class="btn btn-primary" style="margin-top:0;">
          </div>
          </form>
